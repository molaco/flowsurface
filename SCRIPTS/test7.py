...
class AgentLogger:
    ...
    Always displays the last 3 lines of output.
    ...
    def get_renderable(self):
        """Get output as a Rich renderable - always shows last 3 lines."""
        # Get last 3 lines of output
        if not self.lines:
            output = "[dim]Waiting...[/dim]"
        else:
            # Take last 3 lines
            last_lines = self.lines[-3:]
            output = '\n'.join(last_lines)

        # Build display with indicators
        task_name = f"Task {self.task_id}" if self.task_id else "Task"

        # Show line count if we have more than 3 lines
        line_info = ""
        if len(self.lines) > 3:
            line_info = f" [dim]({len(self.lines)} lines)[/dim]"

        # Combine header and content
        header = f"[cyan]{task_name}:[/cyan]{line_info}"

        # Build final display
        content = header + "\n" + output

        # Return Panel - let content control size naturally
        return Panel(
        ...
        for batch_num, batch in enumerate(batches, 1):
            ...
            def create_display():
                ...
                for task_doc in batch:
                    ...
                    renderables.append(logger.get_renderable())
                return Group(*renderables)

            with Live(create_display(), refresh_per_second=2, console=console, auto_refresh=False) as live:
                ...
                # Run display updater and tasks concurrently
    ...
    if args.step in ["1", "3", "all"]:
        if args.impl:
            ...
            if not impl_path.exists():
                console.print(f"[red]✗[/red] Error: IMPL.md not found at {impl_path}")
                return
