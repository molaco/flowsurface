...
def main():
    try:
        ...
        # Add to user_prompt_submit.py around line 165:
        if "/main" in prompt:
            # Create response directory for this session
            response_dir = Path(f".claude/responses/{session_id}")
            response_dir.mkdir(parents=True, exist_ok=True)

            # Write session ID to file for agents
            with open(".claude/.session", "w") as f:
                f.write(session_id)
