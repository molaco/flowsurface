...
async def generate_prompts(objective: str, prompt_writer: str, output_style: str, notify: bool, tts_volume: float) -> dict:
    ...
    async with ClaudeSDKClient(options=options) as client:
        ...
        # If no code blocks found, try to find YAML structure (objective: ... prompts:)
        if not yaml_content:
            # Look for lines starting with 'objective:' or 'prompts:'
            lines = full_response.split('\n')
            yaml_lines = []
            in_yaml = False
            for line in lines:
                # Start capturing when we see objective: or prompts:
                if line.strip().startswith('objective:') or line.strip().startswith('prompts:'):
                    in_yaml = True

                # Stop if we hit markdown headers or obvious non-YAML content
                if in_yaml:
                    if line.strip().startswith('#') or line.strip().startswith('**'):
                        break
                    yaml_lines.append(line)

            yaml_content = '\n'.join(yaml_lines).strip() if yaml_lines else full_response

        try:
            prompts_data = yaml.safe_load(yaml_content)
            if not isinstance(prompts_data, dict):
                print(f"Warning: YAML did not parse to a dict, got {type(prompts_data)}")
                return {"objective": objective, "prompts": []}
            return prompts_data
        except yaml.YAMLError as e:
            print(f"Error parsing YAML: {e}")
            print(f"Raw response: {yaml_content[:500]}...")
            return {"objective": objective, "prompts": []}
