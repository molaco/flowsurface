...
use crate::db::{DatabaseManager, TradesCRUD, KlinesCRUD, DatabaseConfig};
use exchange::Timeframe;
use exchange::adapter::Exchange;

use super::common::fixtures::{create_test_ticker_info, generate_test_trades, generate_test_klines};
use super::common::assertions::{assert_trade_eq, compare_trade_vectors};
...
fn test_klines_persist_and_query_cycle() {
    ...
    // Insert klines
    db.insert_klines(&ticker_info, Timeframe::M1, &klines)
        .expect("Failed to insert klines");
    ...
    let queried_klines = db
        .query_klines(&ticker_info, Timeframe::M1, start_time, end_time)
