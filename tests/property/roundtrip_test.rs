...
use data::db::{DatabaseManager, TradesCRUD, DatabaseConfig};
use exchange::{Trade, Price, Exchange, MinTicksize, TickerInfo, Ticker};
...
fn trade_strategy() -> impl Strategy<Value = Trade> {
    (
        ...
        .prop_map(|(time, is_sell, price, qty)| Trade {
            time,
            is_sell,
            price: Price::from_f32(price).round_to_min_tick(MinTicksize::ZeroPointZeroOne),
            qty,
        })
}
...
proptest! {
    ...
    fn test_trade_roundtrip(trade in trade_strategy()) {
        ...
        let price_diff = (queried_trade.price.as_f32() - trade.price.as_f32()).abs();
    ...
    fn test_price_precision_preservation(price in price_strategy()) {
        ...
        let original_price = Price::from_f32(price).round_to_min_tick(MinTicksize::ZeroPointZeroOne);
        ...
        let retrieved_price = queried[0].price.as_f32();
        let diff = (retrieved_price - original_price.as_f32()).abs();
